...
async def on_message(message):
    ...
    if message.content.startswith(constants.COMMAND_PREFIX):
        message.content = message.content[len(constants.COMMAND_PREFIX):]
        if message.content == 'help':
            await message.channel.send('List of Commands:\n' +
                                       constants.COMMAND_PREFIX + 'help  -  Shows a list of commands\n' +
                                       constants.COMMAND_PREFIX + 'setup -  Creates the designated channel and category for the bot to work\n' +
                                       '\n'
                                       ...
                                       '2. make sure no channels/categories conflict with the bots naming scheme, Currently it is: "' + constants.CATEGORY_NAME + '" and ' + '"' + constants.ANOUNCEMENT_CHANNEL_NAME + '"\n'
                                                                                                                                                                                                                        '3. use ' + constants.COMMAND_PREFIX + 'setup')
            ...
            await message.channel.send(
                constants.COMMAND_PREFIX + message.content + ' is not a valid command. Use ' + constants.COMMAND_PREFIX + 'help for a list of valid commands')
    else:
        return
...
async def setup_local(message):
    ...
    await announcement_local(guild,
